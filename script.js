...
function renderCart() {
  ...
  const tot = cartTotal();
  if (totalSavings > 0) {
    html += '<div style="display:flex;justify-content:space-between;padding:14px 0 4px;font-size:14px;font-weight:700;color:var(--moss)"><span>Savings</span><span>-$' + totalSavings + '.00</span></div>';
  }
  html += '<div style="display:flex;justify-content:space-between;padding:6px 0;font-size:13px;color:var(--bark-light)"><span>Shipping</span><span style="color:var(--moss);font-weight:700">FREE</span></div>';
  html += '<div style="display:flex;justify-content:space-between;padding:8px 0 0;font-family:\'Playfair Display\',serif;font-size:20px;font-weight:700;color:var(--bark);border-top:1px solid var(--linen);margin-top:8px"><span>Subtotal</span><span>$' + tot + '.00</span></div>';
