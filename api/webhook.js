...
async function getCJAccessToken() {
  const res = await fetch('https://developers.cjdropshipping.com/api2.0/v1/authentication/getAccessToken', {
    ...
    body: JSON.stringify({
      apiKey: process.env.CJ_API_KEY, // ✅ apiKey is correct per CJ docs
    }),
  });
...
async function placeCJOrder({ accessToken, paymentIntent }) {
  ...
  // ✅ FIX 1: Updated to createOrderV2 endpoint
  // ✅ FIX 2: Added platformToken header (can be empty string if not required)
  // ✅ FIX 3: Added shippingAddress2 field
  const res = await fetch('https://developers.cjdropshipping.com/api2.0/v1/shopping/order/createOrderV2', {
    method: 'POST',
    headers: {
      'Content-Type':    'application/json',
      'CJ-Access-Token': accessToken,
      'platformToken':   '', // not required for independent sellers
    },
    body: JSON.stringify({
      ...
      shippingAddress2:     addr.line2       || '', // ✅ FIX 3: added line2 support
